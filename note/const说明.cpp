1、摘自《effective C++》item 3：
const 语法虽然变化多端，但并不莫测高深。如果关键字 const出现在星号左边，表示被指物是常量:如果出现在星号右边，表示指针自身是常量:如果出现在
星号两边，表示被指物和指针两者都是常量。
...
constint nValue；         //nValue是const

intconst nValue；    // nValue是const
...
void function(constint Var); //传递过来的参数在函数内不可以改变(无意义，因为Var本身就是形参)
...
(1) constint fun1() 这个其实无意义，因为参数返回本身就是赋值。

(2) constint* fun2() 

调用时 constint*pValue = fun2();
